...
const CreatePost = () => {
  ...
  const generateImage = async () => {
    if (form.prompt) {
      try {
        ...
        const response = await fetch('https://dalle-clone-8uma.onrender.com/api/v1/dalle', {
          method: 'POST',
          headers: {
        ...
        setForm({ ...form, photo: `data:imaage/jpeg;base64,${data.photo}` })
      } catch (error) {
        alert(error);
      } finally {
  ...
  const handleSubmit = async (e) => {
    // ensure browser doesnt reload the application
    e.preventDefault();
    ...
    if (form.prompt && form.photo) {
      ...
      try {
        ...
        const response = await fetch('https://dalle-clone-8uma.onrender.com/api/v1/post', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
    ...
    <section className='max-w-7xl mx-auto'>
      ...
      <form className='mt-16 max-w-3xl' onSubmit={handleSubmit}>
        <div className='flex flex-col gap-5'>
          ...
          {/* place where ai generated image will be shown and it is place where preview 
          of that generated image will be shown */}
